...
def test_async_failure_fallback() -> None:
    ...
    execution_arn = step_function.start_execution(
        machine_arn,
        mock_test_case="AsyncFailureWithFallback",
    ...
    with open(join(dirname(__file__), "output_samples", "test_enrich_async_failure_fallback.json")) as f:
