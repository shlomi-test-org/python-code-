...
def test_async_timeout_fallback() -> None:
    ...
    execution_arn = step_function.start_execution(
        machine_arn,
        mock_test_case="AsyncTimeoutWithFallback",
    ...
    with open(join(dirname(__file__), "output_samples", "test_enrich_async_timeout_fallback.json")) as f:
