...
def test_enrich_pass_not_executions() -> None:
    ...
    execution_arn = step_function.start_execution(
        machine_arn,
        mock_test_case="NoEnrichmentNoExecutions",
        payload={"should_enrich": False},
    )
    history = step_function.get_history(execution_arn)
    redacted_history = step_function.get_redacted_execution(history)
    with open(join(dirname(__file__), "output_samples", "test_enrich_pass_not_executions.json")) as f:
